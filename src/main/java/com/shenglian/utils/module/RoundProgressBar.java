...
public class RoundProgressBar extends View {
    ...
    protected void onDraw(Canvas canvas) {
        ...
        int roundRadius = getHeight() / 2;
        int radius = (int) (roundRadius - roundWidth / 2);
        paint.setColor(roundColor);
        paint.setStyle(Paint.Style.STROKE);
        paint.setStrokeWidth(roundWidth);
        paint.setAntiAlias(true);
        canvas.drawCircle(centre, roundRadius, radius, paint);
        ...
        if (textIsDisplayable && percent != 0 && style == STROKE) {
            canvas.drawText(percent + "%", centre - textWidth / 2, roundRadius + textSize / 2, paint);
        }

        paint.setStrokeWidth(roundWidth);
        paint.setColor(roundProgressColor);
        RectF oval = new RectF(centre - radius, roundRadius - radius, centre
